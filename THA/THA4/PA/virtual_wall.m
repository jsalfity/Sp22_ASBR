function [q_curr] = virtual_wall(p_tip, p_goal, q_init, ...
    q_ub, q_lb, tol,n,d_wall)
...
%     hold on
    ...
    J = J_body(panda, q_init);

    J_alpha = J(1:3, :);    % rotational
    J_eps = J(4:6, :);      % translation

    b2 = -(d_wall-n*t);
    A2 = -n*R*(-vec_2_skew_mat([0;0;-0.1])*J_alpha + J_eps);

    A = blkdiag(A*J,A2);
    B = [b;b_2];
    
    options = optimoptions('lsqlin','Algorithm','active-set','MaxIterations',2000,'TolCon',1e-20);
    q_delta = lsqlin(C, -d(1:3), A, B, [], [], q_lb-q_init, q_ub-q_init,zeros(7,1),options);
