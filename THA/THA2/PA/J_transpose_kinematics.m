function [q, idx, e] = J_transpose_kinematics(robot, Ti, Tf, q0, alpha, max_iterations)
...
q_dot = zeros(max_iterations+1, 7);

e = zeros(1,max_iterations+1);

while (idx <= max_iterations) && (norm(omega) > getGlobaleps || norm(v) > getGlobaleps)
    q_dot(idx+1, :) = J_body(robot,q)'*K*V_b;
    q = trapz(q_dot);
